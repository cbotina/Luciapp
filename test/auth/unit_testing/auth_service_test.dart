...
void main() {
  ...
  group("(AuthService)", () {
    ...
    test("[CP-012] Sucessfull Google login of existing user", () async {
      when(authRepository.loginWithGoogle).thenAnswer(
        (invocation) => Future.value(AuthResult.success),
      );
    ...
    test("[CP-013] Failed Google login", () async {
      when(authRepository.loginWithGoogle).thenAnswer(
    ...
    test("[CP-014] Aborted Google login", () async {
      when(authRepository.loginWithGoogle).thenAnswer(
    ...
    test("[CP-015] Sucessfull Facebook login of new user", () async {
    ...
    test("[CP-016] Sucessfull Facebook login of existing user", () async {
      when(authRepository.loginWithFacebook).thenAnswer(
        (invocation) => Future.value(AuthResult.success),
      );
    ...
    test("[CP-017] Failed Facebook login", () async {
      when(authRepository.loginWithFacebook).thenAnswer(
        (invocation) => Future.value(AuthResult.failure),
      );
  ...
  test("[CP-020] Successfull register", () async {
    final newUser = User(
      userId: "5678",
      name: "Alice",
      age: 21,
  ...
  test("[CP-021] Failed register", () async {
    final newUser = User(
      userId: "5678",
      name: "Alice",
      age: 21,
  ...
  test("[CP-022] Get existing userId", () {
    when(() => authRepository.userId).thenAnswer((_) => '1234');
    final service = AuthService(
      authRepository: authRepository,
      usersRepository: usersRepository,
